...
|---|---|---|
| **answer quality** (LLM judge, `gpt-5.6-sol`, medium reasoning by default) | `correctness` | 0–1 vs. the golden `reference_answer` (answer cases only) |
...
One LangSmith **example = one conversation**. The harness allocates one fresh
LangGraph `thread_id` per conversation and plays every turn through the same
`GraphEngine`. The graph checkpointer carries messages and refusal boundaries
between turns. Scripted history for single-turn examples is seeded directly into
the graph state. No eval writes conversation history under `data/`.
...
* **`simulated`** — a persona (`simulated_user.system`) plus a fixed `opening`,
  ...
  callback maps the simulator's `thread_id` to the graph thread and calls the
  engine. The simulated user reacts to what the system actually said, which can
  expose safety failures under repeated pressure. `stop_when` is an optional
  natural-language stopping condition, judged by the same judge model. Simulated
  conversations have no per-turn references, so only the conversation-level
...
| | `history_used_rate` | diagnostic: follow-up turns whose graph result used messages from earlier turns |
