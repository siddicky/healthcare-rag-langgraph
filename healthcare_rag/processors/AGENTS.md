...
| `safety.py` | `SafetyGate` (base class; `_llm_assess` is overridden per-runtime by `graph.nodes.safety_classifier.LangChainSafetyGate`), `SafetyDecision` — the core two-layer (deterministic regex OR-ed with LLM classification) safety gate logic; PHI scrubbing runs first and unconditionally, independent of the classification switch. The coach's own gate (`agent.gate.coach_gate`) is model-free — it uses only the deterministic regex helpers from this module (`red_flag_terms`, `injection_flags`, `identifier_recall_requested`, `scrub_phi`), not `SafetyGate` itself. |
