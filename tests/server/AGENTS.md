...
## Purpose
Tests for the OSS Agent Server (`server/`) — a clean-room reimplementation of
the LangGraph Agent Server surface (auth/perimeter topology, threads/runs/crons
engines, dual-backend storage seam `SERVER_STORAGE=memory` | `postgres` via
`server/registries.py` + `server/storage.py`). Unit tests here exercise
`server/` in isolation across both backends; the `contract/` and `oracle/`
subdirectories together form the parity gate that proves `server/` behaves
identically to the real `langgraph-api` for the endpoints it implements. The
`postgres` backend is covered by its own gated lane (`POSTGRES=1` /
`POSTGRES_TEST_DSN`, see below) — that lane needs a live Postgres, while the
default `make test` / `make server-test` path stays fully offline.
...
| `test_cors.py` | CORS outermost wrapping verified — preflight `OPTIONS` bypass auth, every response including `401` carries CORS headers |
| `test_crons.py` | Cron engine behaviour (creation, ownership, scheduling contract) |
| `test_crons_postgres.py` | Cron lifecycle against durable `hc_crons` (`reconcile_crons`, `run_due_crons`, advisory claim) — gated on `POSTGRES_TEST_DSN` |
...
| `test_postgres_durability_gaps.py` | Integrated restart durability gaps (thread + checkpoint + run + store + cron survive rebuild, `AsyncPostgresStore` equivalence, erasure survives restart, orphan check, migration idempotency) — gated on `POSTGRES_TEST_DSN` |
| `test_registries.py` | `Registry` protocol contract for both backends (`MemoryRegistry` vs `PostgresRegistry` + `hc_*` tables) — gated on `POSTGRES_TEST_DSN` for the Postgres param |
| `test_runs.py` | Runs engine behaviour |
| `test_runs_durable.py` | Durable run lifecycle (`PERSISTED_PAYLOAD_REDACTION` divergence, `reconcile_interrupted_runs`, terminal-run persistence) — gated on `POSTGRES_TEST_DSN` |
...
| `test_storage_postgres.py` | `create_storage` Postgres branch (`AsyncPostgresSaver`/`AsyncPostgresStore` + `PostgresRegistries` + vector/embedding fallbacks) — gated on `POSTGRES_TEST_DSN` / `POSTGRES=1` |
| `test_threads.py` | Threads engine behaviour (CRUD, ownership, search) |
| `test_threads_postgres.py` | Thread CRUD + expiry purge against durable `hc_threads` — gated on `POSTGRES_TEST_DSN` |
...
- Postgres-gated tests (`test_registries.py` postgres param, `test_threads_postgres.py`, `test_runs_durable.py`, `test_crons_postgres.py`, `test_storage_postgres.py`, `test_postgres_durability_gaps.py`) are gated by `POSTGRES_TEST_DSN` (or `POSTGRES=1` with `DATABASE_URI` for `test_storage_postgres.py` — see `tests/server/conftest.py:postgres_url`); they cleanly skip when unset, so `make server-test` / `make test` stay fully offline. Run the live lane with `make server-test-pg` (compose-provided `POSTGRES_TEST_DSN=postgresql://postgres:postgres@127.0.0.1:55433/postgres`) or manually with `POSTGRES_TEST_DSN=... uv run pytest ...` — see `Makefile:server-test-pg` for the exact file list. The postgres lane uses `server/registries.py` (`PostgresRegistry`/`PostgresRegistries` on `hc_threads`/`hc_runs`/`hc_crons`) and `server/storage.py`'s `postgres` branch (`AsyncPostgresSaver`/`AsyncPostgresStore`).
...
```
make server-test    # unit tests only, fully offline (this directory minus contract/oracle; postgres tests skip without POSTGRES_TEST_DSN)
make server-test-pg # Postgres-gated lane via compose: sets POSTGRES_TEST_DSN=postgresql://postgres:postgres@127.0.0.1:55433/postgres and runs test_registries.py, test_threads_postgres.py, test_runs_durable.py, test_crons_postgres.py, test_storage_postgres.py (+ pg_lane_concurrent.py advisory-lock check); needs docker
...
POSTGRES_TEST_DSN=postgresql://postgres:postgres@127.0.0.1:55433/postgres uv run pytest tests/server/test_registries.py tests/server/test_threads_postgres.py tests/server/test_runs_durable.py tests/server/test_crons_postgres.py -q
# make test / make server-test remain fully offline — no Postgres, no network; the postgres lane is opt-in via POSTGRES_TEST_DSN / make server-test-pg
```
