...
`healthcare_rag/agent/http_app.py:app`), which no longer runs any startup
validation: member feedback self-locates its LangSmith trace at request time
(`post_feedback` resolves the message's `lc_run--<run_id>` id), so there is no
feedback-project env to pin here. These suites are about server topology, not
about the operator's `.env`.
"""
