...
def test_literal_targets_match_node_constants() -> None:
    ...
    assert set(get_args(routers.GateFanOutTarget)) == {routers.NODE_CLARIFY, routers.NODE_CONTEXT}
    assert set(get_args(routers.MergeTarget)) == {routers.NODE_EVALUATE, routers.NODE_GENERATE}
    assert set(get_args(routers.EvaluateTarget)) == {routers.NODE_GENERATE}
    assert set(get_args(routers.ValidateTarget)) == {routers.NODE_FOLLOW_UPS, routers.NODE_FINALIZE}
    ...
    constants = {
        value for name, value in vars(routers).items() if name.startswith("NODE_") and isinstance(value, str)
    }
...
def test_compiled_edges_match_router_literals() -> None:
    ...
    assert _targets(graph, routers.NODE_EVALUATE) == {routers.NODE_RETRIEVE, routers.NODE_GENERATE}
    assert _targets(graph, routers.NODE_VALIDATE) == set(get_args(routers.ValidateTarget))
...
def test_routers_return_declared_literals() -> None:
    ...
    assert routers.route_after_gate({}) == ["clarify_query", "extract_conversation_context"]
