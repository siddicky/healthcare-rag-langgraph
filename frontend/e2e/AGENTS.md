...
## Purpose
Hermetic Playwright end-to-end tests for the coach frontend. `server.py` is a Python orchestrator that boots the FULL local stack with ZERO external network: a scripted dependency server (fake OpenAI gateway + Supabase stub + LangSmith feedback mirror), a real `langgraph dev` Agent Server running the actual coach graph with one offline seam (the `medical_lookup` tool's RAG child relays to an offline monograph answer graph), and a production `next start` frontend built against that server. The coach's own routing gate is model-free now, so every coach-agent turn — including the model's decision to call `medical_lookup` — runs through the scripted OpenAI gateway; `FixtureHandler._agent_turn` emits a `medical_lookup` tool call whenever the turn's question mentions a drug (`metformin`/`lipitor`). `smoke.spec.ts` drives two member identities (`u1`, `u2`) through the full product surface — chat, generative-ui cards, interrupts, reminders, document upload, regenerate, feedback, branching — against this stack. `run.ts` holds shared helpers (runfile reading, envelope extraction, date math). `global-setup.ts`/`global-teardown.ts` start/stop `server.py` around the whole Playwright run.

## Key Files
| File | Description |
|------|-------------|
| `server.py` | The hermetic orchestrator: scripted OpenAI-shaped gateway, Supabase stub, LangSmith feedback mirror, spawns `langgraph dev` with one offline seam (`rag_relay.child`), spawns `next start`; writes readiness + connection info to a JSON runfile; tears everything down on SIGTERM/SIGINT/exit |
...
- `server.py`'s offline seam (`rag_relay.child`) is injected at import time into a generated graph module — this is the same pattern as `scripts/coach_smoke.py`/`scripts/deployed_smoke.py` at the repo root; keep this suite's seam consistent with those if the coach graph's public interface changes.
