...
## Purpose
The coach chat domain layer: the member-perimeter wire protocol (`coachProtocol.ts`), the HTTP/SDK client (`coachApi.ts`, wired to production deps in `coachClient.ts`), the LangChain-serialized message model and turn-splitting logic (`model.ts`), the updates-only stream reducer (`stream.ts`), the client-driven erase flow (`erase.ts`), the document upload state machine (`uploadFlow.ts`), local thread titles (`titles.ts`), and the `useCoachStream` controller hook that ties all of it into the UI. The transport is CopilotKit v2 headless: `coachClient.ts` injects `useCopilotKitCoachStream` (an AG-UI `useAgent` engine projected onto the wire model) as the deps' `useStream`, and `renderers/` registers the per-tool cards (`useRenderTool`) + interrupt surface (`useInterrupt`) mounted once inside the provider by `<CoachToolRenderers/>`. `components/` holds the presentational React for the chat screen (composer, message list, sidebar, action bar, interrupt panel). Everything network-facing is injected as a `CoachChatDeps` bundle so tests can drive the whole controller from scripted fakes with zero real network or timers.
...
| `coachClient.ts` | `createBrowserDeps()` — production wiring: a `CoachFetch` bound to the Supabase-refreshed bearer, the CopilotKit headless engine (`useCopilotKitCoachStream` from `useCoachStream.ts`), real `crypto.randomUUID()`/`setTimeout`, and the poll intervals (erase: 1500ms/40; upload: 1200ms/50). Memoized singleton; tests never touch this |
...
| `stream.ts` | `applyStreamPart()` — pure reducer folding one SDK stream event into `{messages, interruptValue}`; only `RENDERED_NODE_NAMES` nodes are read, human echoes from the stream are dropped; also hosts `chatTelemetry`/`chatTelemetrySink`. The scripted test harness (`__tests__/helpers.ts`) folds fake stream parts through it |
...
| `useCoachStream.ts` | The controller hook + transport engines: owns all chat state (threads, messages, pendingInterrupt, busy, upload, erase, feedback), exposes `send`/`attach`/`approveInterrupt`/`regenerate`/`branch`/`sendFeedback`/`newConversation`/`selectThread`/`removeThread`/`signOut`; takes a `CoachChatDeps` bundle for full dependency injection. Also exports `useCopilotKitCoachStream` (the live AG-UI engine over `useAgent`: `runAgent`/`connectAgent`/`detachActiveRun`/`abortRun`, `projectAgentMessages` projection) and the clipboard frontend tool |
| `renderers/` | CopilotKit v2 per-tool renderers registered via `useRenderTool` (`compose_ui` catalog pipeline, envelope tools, medical verbatim bubble, catch-all, interrupts via `useInterrupt`, clipboard via `useFrontendTool`); mounted once as `<CoachToolRenderers/>` in `app/chat/page.tsx` (see `renderers/` modules) |
| `components/` | Presentational React for the chat screen (see `components/AGENTS.md`) |

## For AI Agents

### Working In This Directory
- Every network/timer/random seam in `useCoachStream` goes through the injected `deps: CoachChatDeps` — never call `fetch`, `setTimeout`, or `crypto.randomUUID()` directly inside this hook or its helpers; add the seam to `CoachChatDeps` instead.
...
### External
- `@langchain/langgraph-sdk` (`Client`, stream types), `@copilotkit/react-core/v2/headless` (`useAgent`, `useRenderTool`, `useInterrupt`, `useFrontendTool`), `zod` 4
