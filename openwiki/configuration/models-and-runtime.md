...
| `HC_RAG_RETRIEVER` | `weaviate` | retrieval arm: `weaviate`, `pageindex` (LLM tree-search), or `pinecone` (serverless hybrid). Swaps only the per-collection search callable — see [retrieval arms](../retrieval/retrieval-arms.md) |
