...
| change auth principals, Studio access, or the member perimeter | [Coach agent](agent/coach.md) | `supabase_bearer`, `_is_studio`, `MemberPerimeterMiddleware`, `prepare_thread_deletion`, `disable_studio_auth` | `make test` (`tests/agent/test_perimeter_studio.py`, `test_perimeter_composed.py`, `test_auth.py`); `make deployed-smoke` |
