...
`healthcare_rag/processors/` now holds the reusable logic the nodes call: `safety.py` + `safety_patterns.py` (`injection_flags`, `strip_injection`) + `safety_signals.py` (`scrub_phi`, `contains_phi`, `identifier_recall_requested`, `red_flag_terms`) together are "the safety gate" ([safety gate](../safety/gate.md)); `safety_responses.py`; `social_responses.py` (benign-social direct text); `privacy.py` + `privacy_patterns.py` (the [PrivacySanitizer](../privacy/sanitizer.md)); `direct_output_policy.py` (tool-arm answer gating); `refusal_boundary.py`; `validation.py` + `validation_citations.py` + `validation_source.py` + `validation_rendering.py` ([answer validation](validation.md)); `retrieval.py`; `generation.py`; `pdf_chunker.py`; and the alternative retrieval arms `pageindex_retrieval.py`/`pinecone_retrieval.py` plus `rerank.py` ([retrieval arms and reranking](../retrieval/arms-and-reranking.md)). `base.py` only provides the `log_timing` decorator. The old `PromptManager`/`LLMParserService`/processor-class layer is gone. Full per-file map: `healthcare_rag/processors/AGENTS.md`.
