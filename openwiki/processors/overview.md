...
`healthcare_rag/processors/` now holds the reusable logic the nodes call (`safety.py`, `safety_responses.py`, `social_responses.py` (benign-social direct text), `privacy.py` + `privacy_patterns.py` (the [PrivacySanitizer](../privacy/sanitizer.md)), `direct_output_policy.py` (tool-arm answer gating), `refusal_boundary.py`, `validation.py`, `retrieval.py`, `generation.py`, `pdf_chunker.py`, and the alternative retrieval arms `pageindex_retrieval.py`/`pinecone_retrieval.py` plus `rerank.py` ([retrieval arms and reranking](../retrieval/arms-and-reranking.md))); `base.py` only provides the `log_timing` decorator. The old `PromptManager`/`LLMParserService`/processor-class layer is gone.
