...
- `scrub_phi(text, extra_spans)` (`healthcare_rag/processors/safety_signals.py`) replaces identifiers with typed redaction tokens. It runs no regexes of its own: it delegates to the process-wide [PrivacySanitizer](../privacy/sanitizer.md) (`get().privacy.scan`), and the `extra_spans` argument is accepted only for call compatibility (it is discarded) — model output never receives text-mutation authority. It is **fail-closed**: initialization or scan errors raise `PrivacyScanError` and abort the turn, so `contains_phi` reflects only what the deterministic sanitizer found, independent of the LLM call.
- The `safety_gate` node first derives history views from the checkpointed messages: when the gate is on, every message is scrubbed (`build_history_views`), then token-capped (`HC_RAG_HISTORY_MAX_TOKENS`) and paired into the processed-history and context windows (`healthcare_rag/graph/history.py`). The node then **resets all downstream state** for the turn (retrievals, branch events, route, generation, validation, `direct_response`, `response_action`, `query_router`, etc., via `Overwrite`) so nothing leaks from a prior turn on a checkpointed thread, then either replays a matching refusal boundary or runs `LangChainSafetyGate.evaluate` (`graph/nodes/safety.py`, adapter in `nodes/safety_classifier.py`). On short-circuit, `finalize` (`graph/nodes/safety_finalize.py`) joins notices + the templated body with `follow_ups = []` deliberately; the refusal never re-enters retrieval or generation. A benign-social direct answer (`direct_response`) also finalizes without retrieval.
- Non-short-circuited queries run the pipeline on the scrubbed query (`working_query = decision.scrubbed_query`); one-line notices ("identifiers disregarded", "instructions unchanged") are prefixed to the final answer by `finalize`.
- Observability: the `safety` state field (`SafetyOutcome`) records category, `contains_phi`, `short_circuited`, response kind, deterministic flags, PHI kinds, LLM-call count, `benign_social`/`social_intent`, `classifier_backend`/`classifier_calls`/`embedding_calls`, gate latency, boundary-hit info, and a scrubbed rationale; the graph copies it into each eval result row as `safety_outcome`.

## Benign-social direct responses and the query-response arms

The gate's `SafetyAssessment` carries `benign_social` + `social_intent` (`greeting|thanks|goodbye|capability`), accepted by `SafetyGate.assess` only when the merged category is `out_of_scope` and the turn is not escalated. How that turn is answered is decided by `HC_RAG_QUERY_RESPONSE_ARM` (`services/models.py::query_response_arm`, default `current`):

- `current` — out-of-scope template refusal, as before;
- `deterministic` — a hard-coded `social_response(intent)` text becomes the `direct_response` and finalizes immediately;
...
`HC_RAG_SAFETY_CLASSIFIER` (`llm` default | `semantic_router`, `services/models.py::safety_classifier_backend`) selects the classification backend. The `semantic_router` backend was **never installed, imported, or exercised**: `semantic-router==0.1.16` is unsatisfiable with the unchanged `openai>=1.76,<2` and `python-dotenv>=1.1` bounds, so the semantic lane is dependency-INCONCLUSIVE — a dependency fact, not a runtime or quality result (see [routing evaluations](../observability/routing-evals.md)). Selecting it is also **fail-hard, not fail-open, at process start**: `GraphEngine.__init__` raises `SafetyClassifierUnavailableError` immediately if `settings.safety_classifier == "semantic_router"`, so a misconfigured deployment cannot silently run without a classifier — it refuses to start. Production defaults remain `current`/`llm`.

## Category-to-decision mapping

`SafetyGate._evaluate` maps the merged outcome to exactly one of five short-circuiting decisions, or lets the turn continue:

- `prompt_injection` and unsalvageable (or a second failed pass) → `injection_response()`.
- `emergency_red_flag` → `emergency_response(overdose=...)`, where `overdose` is set when the deterministic `possible_overdose` red flag fired.
- an identifier-recall request (`identifier_recall_requested(query)`, checked independently of the LLM category) → `identifier_recall_response()`.
- `personal_medical_advice` → `personal_advice_response()`.
- `out_of_scope` → `out_of_scope_response()` (or a benign-social direct answer, per the arm above).
- `in_scope_informational` or `ambiguous` → no short-circuit; the pipeline runs on the scrubbed query.
...
- `emergency_response(*, overdose=False)` opens with `"What you're describing needs to be assessed by a person, not by a document assistant."`, always includes `"Please seek urgent medical care now: call your local emergency number (911 in Canada and the US) or go to your nearest emergency department."`, and when `overdose=True` inserts `"If you think you have taken too much of a medicine, call your local poison control centre on your way — they can advise while you travel."` It deliberately contains **no monograph content**.
- `personal_advice_response()` opens with `"I can't tell you what to do with your own dose or treatment."` and redirects to `"the person who prescribed it — your doctor or nurse practitioner, your pharmacist, or your diabetes nurse"`.
- `out_of_scope_response()` opens with `"I can only answer from two documents: the Lipitor (atorvastatin) and metformin product monographs."`
- `identifier_recall_response()` opens with `"I don't hold personal identifiers."` and states identifiers are `"stripped out of a message before it is used and are not kept in this conversation"`.
- `injection_response()` opens with `"I can't change the instructions I operate under, adopt a different persona, or print those instructions for you"`.
- `PHI_NOTICE`: `"I've disregarded the personal identifiers in your message; please don't share them here."` — prefixed as its own line whenever `scrub_phi` finds an identifier.
- `INJECTION_NOTICE`: `"I can't change the instructions I operate under or take on a different persona, so I'll answer your question as the monograph assistant."` — prefixed only on a salvaged `ignore_instructions` pass.

Hard rule: **no template contains a specific number with a clinical unit**. `NUMERIC_DOSE` (`safety_patterns.py`) matches numbers with clinical units (`mg`, `mcg`, `ml`, `mmol/L`, `%`, `tablets`, `times a day`, `hours`, `days`, `weeks`); `tests/test_safety_gate.py::test_no_template_contains_a_specific_dose` and `tests/graph/test_graph_safety.py::test_refusal_templates_never_contain_a_numeric_clinical_unit` both assert it against every string in `ALL_TEMPLATES` (`safety_responses.py`), mirroring the `evals.evaluators.numeric_advice_leak` grader. Every string checked — the five refusal bodies plus the standard and overdose emergency variants plus both notices — is confirmed clean by these tests, so no shipped refusal/fallback text can leak a dosing number.

## Terminal refusals and informational follow-ups

`personal_medical_advice` refusals are terminal for the turn they occur in, but a later, explicitly informational question on the same topic remains answerable: the refusal-boundary matcher's informational carve-out (`_INFORMATIONAL` in `refusal_boundary.py`) sends that new turn through the full safety gate and normal RAG pipeline instead of replaying the boundary.
...
`HC_RAG_REFUSAL_BOUNDARY` (default `true`; `refusal_boundary_enabled()` in `services/models.py`, read live each turn) makes the gate persist qualifying refusals into checkpoint state (`RAGState.refusal_boundaries`) and replay them without a second LLM call (`healthcare_rag/processors/refusal_boundary.py`; `graph/nodes/safety.py`):
...
- **Read path:** on the next turn, before the LLM call, `boundary_hit(scrubbed_query, boundaries)` matches under exclusive cue precedence — emergency red flags first, then unsalvageable injection flags, then first-person dosing/decision cues — and only if the query is not informational (`_INFORMATIONAL` carve-out) and its topic matches (anaphoric ≤15-word or referent queries inherit the stored topic; `both`↔single-drug cross-match). A hit short-circuits with `response_kind="boundary_replay"`, `llm_calls=0`, route `safety_gate:boundary:<kind>`, and replays the stored template.
- **Safety valves:** corrupted or stale-template-version boundaries are parsed as inert (`RefusalBoundary.from_state` rejects any response not in the current allowed set, and any `template_version` other than the current one), so corrupted checkpoint state can never widen behavior beyond the fixed template set. Turning the gate off (`HC_RAG_SAFETY_GATE=false`) never writes and never replays a boundary.
- **Concurrency limit:** same-thread concurrent turns are unsupported by design; the CLI and eval harness run turns sequentially (module docstring, `refusal_boundary.py`).

Focused tests: `tests/test_refusal_boundary.py` (pure matching/upsert/topic semantics), `tests/graph/test_graph_safety.py` boundary suite (write/replay/carve-out/knob-off/gate-off/corrupted/variant mismatch), and `tests/graph/test_boundary_durability.py` (SQLite checkpoint survives reopen with no raw PHI bytes).

## Flags and linear path

`HC_RAG_SAFETY_GATE` (default `true`) or `HC_RAG_DISABLE_STAGES=safety` turns classification off for before/after ablations (`safety_gate_enabled` in `healthcare_rag/services/models.py`); identifier scrubbing remains active and the scrubbed question still becomes the working query.

**Change guidance:** edit deterministic regex patterns in `safety_patterns.py`/`safety_signals.py`, merge policy in `safety.py`, or templates in `safety_responses.py` (and remember `refusal_boundary.py`'s allowed-response set and `TEMPLATE_VERSION` must be bumped/kept in sync when a template string changes — old boundaries go inert by design); recognizer changes belong in `privacy.py` and must move with the pinned spaCy model in the app image (see [sanitizer](../privacy/sanitizer.md)). Keep every deterministic check escalate-only, and validate with `make test` (`tests/test_safety_gate.py` covers injection, red flags, template content, and gate policy cases; `tests/graph/test_graph_safety.py` the graph wiring, terminal refusals, ablation switch, and boundary write/replay). Then measure with evaluations: watch `safe_redirect`/`numeric_advice_leak`/`pii_persistence` improve while `correctness`/`groundedness`/`chunk_recall` stay flat. Known limits (deterministic floor not fence, first-person red flags, LangSmith holding scrubbed text, residual drift) are tracked in [safety posture](posture.md).
