...
Tracing is disabled unless `LANGSMITH_TRACING` (or `LANGCHAIN_TRACING_V2`) is `1`, `true`, or `yes`. With it enabled and `langsmith` installed, `wrap_openai_client` wraps the shared OpenAI client and `rag_stage` decorates the root processing and clarify, decompose, retrieve, evaluate, context extraction, answer, validation, and follow-up child stages. Without it, helpers are no-ops and missing `langsmith` merely logs a warning (`healthcare_rag/services/tracing.py#L32-L102`). `LANGSMITH_API_KEY` is required for remote use; `LANGSMITH_PROJECT` is optional. Retrieval traces are rendered as `documents` with content plus source, UUID, score, pages, and routed query (`#L74-L97`).

`QueryMonitor` signals the **first raw generated answer** and the **first completed branch** for interactive timing. Neither is necessarily the ultimately selected branch: the orchestrator may later choose a branch with higher refinement traits, and the CLI replaces monitor final answer with that selection (`healthcare_rag/orch/monitor.py#L82-L107`; `healthcare_rag/cli/interactive.py#L39-L44`). Do not report a raw/first-completed event as the verified final answer.
...
`build_rag` uses production setup but replaces history with a temporary directory. `run_one` seeds supplied history, runs the real orchestrator, and returns final/raw answer, contexts, chunk/page/source IDs, selected branch data, latency, usage, the caught error, and the orchestrator's `safety_outcome` record (gate category, `contains_phi`, `short_circuited`, `response_kind`, deterministic flags, `gate_latency_s`) for per-example gate auditing (`evals/harness.py#L61-L121`, `#L150-L257`). It instruments OpenAI `chat.completions.create` and `beta...parse`; a `contextvars` usage sink propagates into child asyncio tasks so calls from a request are collected together (`evals/harness.py#L61-L121`, `#L150-L256`). It never writes production conversations.
...
The runner requires `OPENAI_API_KEY` and `LANGSMITH_API_KEY`; it uploads experiments even with tracing off, but per-stage cost attribution needs tracing. Reports land at `evals/results/<experiment>.json` and `.md`, with overall/category aggregates, p50/p95, and LangSmith stage-cost tree (`evals/run_baseline.py#L64-L194`; `evals/report.py#L163-L276`). `evals/results/` is excluded from inspection. `evals.calibrate` checks labelled evaluator expectations; `--no-judges` is its fast deterministic path (`evals/calibrate.py#L1-L19`). The offline pytest suite in `tests/test_evaluators.py` locks those expectations as regressions (`make test`), and `make test-judges` adds the LLM-judge expectations behind the `judge` marker (needs `OPENAI_API_KEY`). `make test` also runs the orchestrator synthesis suite (`tests/test_orchestrator_synthesis.py`, see [architecture](../architecture/overview.md)) and the scheduler fast-task regression. Add any evaluator's emitted keys to `EVALUATOR_KEYS`, add discovered grader failure to calibration, and add product behavior failures as golden rows. Rescore old comparison runs after evaluator/judge changes to keep feedback surfaces comparable (`evals/evaluators.py#L400-L436`; `evals/rescore.py#L1-L16`).
...
Each conversation has one fresh application user ID and a new orchestrator per sequential turn; state persists only through the temporary `ConversationHistory`, matching CLI semantics. Simulated execution maps simulator `thread_id` to that application `user_id` before invoking the same production turn path (`evals/multiturn_harness.py#L1-L29`, `#L208-L246`). Per-turn telemetry includes `used_history`: it means the history-extraction stage said prior context was required; it does **not** prove prior context caused or improved the answer.
