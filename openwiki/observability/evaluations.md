...
Tracing is disabled unless `LANGSMITH_TRACING` or `LANGCHAIN_TRACING_V2` is the exact lower-case string `true`; `1`, `yes`, and other values do not enable this wrapper (`healthcare_rag/services/tracing.py`). With that opt-in and `langsmith` installed, `wrap_openai_client` wraps OpenAI clients and `traceable`/`rag_stage` can decorate call sites; without it, those helpers are no-ops and a missing `langsmith` package merely logs a warning. `enforce_input_hiding()` turns both tracing variables to `false` when tracing was requested without `LANGSMITH_HIDE_INPUTS=true`; set that hiding flag before enabling tracing. `LANGSMITH_API_KEY` is required for remote use; `LANGSMITH_PROJECT` is optional. In the graph runtime, the root `GraphEngine.run_turn`/`process_query` run has a `process_inputs` hook that **fail-closed scrubs the question** before LangSmith receives it, and `retrieve_documents` creates a `retriever` child run. The tracing service's stage-decorator helper is available for named stages, but its presence alone does not establish that every graph stage has an individual trace. Tests force tracing off unless `HC_RAG_TEST_TRACING=true` (`tests/conftest.py`).
...
The runner requires `OPENAI_API_KEY` and `LANGSMITH_API_KEY`; it uploads experiments even with tracing off, but per-stage cost attribution needs tracing. Reports land at `evals/results/<experiment>.json` and `.md`, with overall/category aggregates, p50/p95, and LangSmith stage-cost tree (`evals/run_baseline.py`; `evals/report.py`). `evals/results/` is excluded from inspection. `evals.calibrate` checks labelled evaluator expectations; `--no-judges` is its fast deterministic path (`evals/calibrate.py`). The offline pytest suite (`make test`) locks those expectations (`tests/test_evaluators.py`, `tests/test_evaluator_calibration.py`), the graph runtime contracts (`tests/graph/`: build/routing/typing/flow/safety/history/state/branch-fold/prompt-fidelity/route-tools/union-results/engine-record/query-or-respond/privacy and the evals-engine contract), the privacy scanner (`tests/test_privacy_sanitizer.py`), the retrieval arms (`tests/test_pageindex_retrieval.py`, `tests/test_pinecone_retrieval.py`, `tests/test_rerank.py`), the routing surfaces (`tests/test_routing_*`, `tests/test_social_responses.py`), and the **parity gate** (`tests/test_parity_gate.py` with `evals/parity.py`, `evals/parity_drills.py`): refactors must reproduce a sealed baseline's measurements — per-metric tolerances, example-ID multisets, git SHA match, multi-turn `turns_completed`, and finite values — and `tests/test_seal_clean.py` keeps baseline sealing honest. `make test-judges` adds the LLM-judge expectations behind the `judge` marker (needs `OPENAI_API_KEY`). Add any evaluator's emitted keys to `EVALUATOR_KEYS`, add discovered grader failure to calibration, and add product behavior failures as golden rows. Rescore old comparison runs after evaluator/judge changes to keep feedback surfaces comparable (`evals/evaluators.py`; `evals/rescore.py`).
